...
Characteristics:
- Higher edge threshold (5%+)
- 2-3 tick persistence requirement
- Moderate cooldown (3 min)
- Controlled position sizing
- Tight edge compression exit
- Low churn target
"""
...
class ModelADisciplined(BaseStrategy):
    """
    Disciplined Edge Trader - Higher conviction, fewer trades.
    ...
    def evaluate_entry(
        ...
        """Evaluate entry with disciplined criteria."""
        ...
        return StrategyDecision(
            ...
            reason=f"Disciplined entry: edge {signal.edge:.2%}, score {signal_score:.0f}",
            market_id=market.id,
            game_id=game.id,
            side=side,
            quantity=size,
    ...
    def evaluate_exit(
        ...
        # 1. Stop loss
        stop_loss_pct = exit_rules.get("stop_loss_pct", 0.10)
        ...
        # 4. Time-based exit
        max_hold_time = exit_rules.get("time_based_exit_seconds", 600)
        ...
        # 5. Trailing stop
        trailing_stop = exit_rules.get("trailing_stop_pct", 0.05)
    ...
    def calculate_size(self, signal: Signal, available_capital: float) -> int:
        """
        Calculate position size using disciplined sizing.
        Base: 2% of capital, scaled with edge.
        """
        sizing = self.config.position_sizing
        
        # Base size
        base_pct = sizing.get("base_size_pct", 0.02)
        base_amount = available_capital * base_pct
        
        # Scale with edge
        if sizing.get("scale_with_edge", True):
            edge_scale = sizing.get("edge_scale_factor", 2.0)
            edge_mult = 1 + (abs(signal.edge) * edge_scale)
            base_amount *= min(edge_mult, 2.0)  # Cap at 2x
        
        # Apply Kelly fraction
        kelly = sizing.get("kelly_fraction", 0.25)
        base_amount *= kelly
