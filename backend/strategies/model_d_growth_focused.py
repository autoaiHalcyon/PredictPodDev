"""
Model D - Growth Focused Trader

Characteristics:
- Moderate edge threshold (4%+)
- 2 tick persistence requirement
- Lower cooldown (2 min)
...
class ModelDGrowthFocused(BaseStrategy):
    """
    Growth Focused Trader - Balanced approach with moderate risk.
    ...
    Exit Rules:
    - Edge compression below 1.5%
    - Profit target: 12%
    - Stop loss: 8%
    - Time-based exit: 7.5 min
    """
    ...
    def evaluate_entry(
        ...
        """Evaluate entry with growth-focused criteria."""
        ...
        min_edge = entry_rules.get("min_edge_threshold", 0.04)
        ...
        return StrategyDecision(
            ...
            reason=f"Growth entry: edge {signal.edge:.2%}, score {signal_score:.0f}",
            market_id=market.id,
            game_id=game.id,
            side=side,
            quantity=size,
    ...
    def evaluate_exit(
        ...
        # 2. Profit target
        profit_target_pct = exit_rules.get("profit_target_pct", 0.12)
        ...
        exit_threshold = exit_rules.get("edge_compression_exit_threshold", 0.015)
        ...
        # 4. Time-based exit
        max_hold_time = exit_rules.get("time_based_exit_seconds", 450)
    ...
    def calculate_size(self, signal: Signal, available_capital: float) -> int:
        """
        Calculate position size using growth-focused sizing.
        Base: 2.5% of capital, scaled with edge.
        """
        sizing = self.config.position_sizing
        
        # Base size
        base_pct = sizing.get("base_size_pct", 0.025)
        ...
        kelly = sizing.get("kelly_fraction", 0.30)
        base_amount *= kelly
        
        # Max position check
        max_pct = sizing.get("max_position_pct", 0.06)
