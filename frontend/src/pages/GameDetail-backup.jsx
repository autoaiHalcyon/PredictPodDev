...
// Applies to NBA, NCAAM, NCAAW, WNBA, NBL, BBL, ABA, EuroLeague, etc.
...
// ── Mock chart seeder ───────────────────────────────────────────────────
// Generates a realistic random-walk probability history from game start → now
// when Kalshi returns no candle history (pre-game / CORS / rate-limit).
function generateMockChartData(startTimeIso, markets) {
  ...
  // Converge toward actual live price (0-1 scale → 0-100)
  const rawPrice = markets[0]?.last_price ?? markets[0]?.yes_price ?? 0.5;
  const endP0    = Math.round(rawPrice * 100 * 100) / 100; // 2-decimal %

  // If game just started (start_time ≈ now or in future), look back 3 h for history
  let start = new Date(startTimeIso);
  if (isNaN(start) || now.getTime() - start.getTime() < 5 * 60 * 1000) {
    start = new Date(now.getTime() - 3 * 60 * 60 * 1000); // fallback: 3 h ago
  }

  const totalMs  = now.getTime() - start.getTime();
  const INTERVAL = 60 * 1000;                                       // 1-min buckets
  const n        = Math.max(10, Math.min(300, Math.ceil(totalMs / INTERVAL)));
  const stepMs   = totalMs / n;

  // Seed random-walk: start near 50/50 then drift to the real current price
  const points = [];
  // Use a seeded-ish start: open midpoint offset by small random bias
  let val = 50 + (Math.random() - 0.5) * 8;

  for (let i = 0; i <= n; i++) {
    const t        = new Date(start.getTime() + i * stepMs);
    const progress = i / n; // 0 → 1

    if (i > 0) {
      // ±5 % Gaussian-ish noise (sum of 3 uniforms approximates normal)
      const noise  = ((Math.random() + Math.random() + Math.random()) / 3 - 0.5) * 10;
      // Pull toward the real ending price more strongly as progress → 1
      const drift  = (endP0 - val) * (0.04 + 0.12 * Math.pow(progress, 1.5));
      val = Math.max(1, Math.min(99, val + noise + drift));
    }

    const v0 = Math.round(val * 100) / 100;          // 2-decimal precision
...
function transformKalshiEvent(eventData) {
  ...
  if (markets.length < 1) throw new Error('No markets found in Kalshi event');
  ...
  if (marketStatus === 'active') {
    const hasTraded = market0.volume > 0 || market1.volume > 0;
  ...
  const makeTeam = (m) => ({
    name: m.name,
    abbreviation: m.rulebook_variables?.['Color Palette']?.split('-')[1]
      || m.name.split(' ').map(w => w[0]).join('').slice(0, 4),
    logo_url: null,
  });

  // Kalshi lists market[0] = home team, market[1] = away team
  const homeTeam  = makeTeam(market0);
  const awayTeam  = makeTeam(market1);
  const homeMarket = market0;
  const awayMarket = market1;
  const homeProb   = normProb0;
  const awayProb   = normProb1;

  const homeMidCents  = (homeMarket.yes_bid + homeMarket.yes_ask) / 2;
  ...
  return {
    game: {
      ...
      status:       gameStatus,
      // start_time ≈ target_datetime - 3h for NBA (target_datetime is expiration/game-end)
    ...
    markets: [
      {
        id:                 homeMarket.id,
        ticker:             homeMarket.ticker_name,
        // ── team_name is the clean display name shown on the card ──
        name:               homeMarket.name,
        team_name:          homeTeam.name,
        outcome:            'home',
        yes_bid:            homeMarket.yes_bid / 100,
        yes_ask:            homeMarket.yes_ask / 100,
        yes_price:          homeMidCents / 100,
        last_price:         homeMarket.last_price / 100,
        prev_price:         (homeMarket.prev_period_price ?? homeMarket.yes_bid) / 100,
      ...
      {
        id:                 awayMarket.id,
        ticker:             awayMarket.ticker_name,
        name:               awayMarket.name,
        team_name:          awayTeam.name,
        outcome:            'away',
        yes_bid:            awayMarket.yes_bid / 100,
        yes_ask:            awayMarket.yes_ask / 100,
        yes_price:          (awayMarket.yes_bid + awayMarket.yes_ask) / 2 / 100,
        last_price:         awayMarket.last_price / 100,
        prev_price:         (awayMarket.prev_period_price ?? awayMarket.yes_bid) / 100,
...
const GameDetail = () => {
  ...
  // ── Seed chart with Kalshi candlestick history (called once on mount) ────
  const loadHistory = useCallback(async (markets) => {
    ...
    try {
      ...
      // Fetch hourly candlesticks via backend proxy (which uses the authenticated trade-api/v2)
      // The old direct call to api.elections.kalshi.com/v1/markets/{ticker}/candlesticks
      // always returns 400 — that endpoint does not exist in the v1 elections API.
  ...
  const loadGame = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (hm) {
        // Seed history once we have the market tickers
        await loadHistory(gameData.markets);

        // If Kalshi returned no candles OR all points are flat, seed with mock random walk
        ...
        // Build a point object with prices for ALL markets (for multi-line chart)
        const point = {
          timestamp: now.toISOString(),
          time:      fmtChartTime(now),
          // Legacy fields (for backward compat with volatility chart)
        ...
        // Add a market_prices object with team name keys for each market
        point.market_prices = {};
      ...
      setLastFetched(now);
      // Default trade panel to the FAVORITE (highest-prob market), like Kalshi does
      if (!silent) {
        const hm = gameData.markets.find((m) => m.outcome === 'home');
  ...
  const handleTrade = async () => {
    ...
    try {
      ...
      // Derive quantity from dollar amount; fall back to legacy tradeQuantity
    ...
    const executeAutoTrade = async () => {
      ...
      try {
        const newTrade = {
          ...
          league:        data?.game?.league || '',
          market_id:     market.id,
          // ── Same fix: store clean team name ──────────────────────────────
          market_name:   teamDisplayName,
          team_name:     teamDisplayName,
          side:          autoSide,
          direction:     'buy',
        ...
        <div className="max-w-[1600px] mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Kalshi link — matches Dashboard "K" button */}
              {(() => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-4">
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-4 sticky top-20 space-y-4">
              ...
              {/* Context line: what you're about to trade */}
              {selectedMarket && (
              ...
              <div className="space-y-3">
                ...
                {/* Odds + Payout (shown when amount entered) */}
                {(() => {
                  const dollarAmt = parseFloat(tradeDollarAmount) || 0;
