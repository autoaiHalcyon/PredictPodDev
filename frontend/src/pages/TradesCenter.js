...
// ─────────────────────────────────────────────────────────────────────────────
// ✅ FIX: Declare API_BASE BEFORE buildKalshiUrl uses it.
// Previously API_BASE was declared with `const` further below, causing a
// Temporal Dead Zone (TDZ) ReferenceError when buildKalshiUrl was called
// during the polling interval — silently returning `undefined/api/kalshi/event/...`
// which made every fetch fail and P&L stay at $0.
const KALSHI_TRADE_API_BASE = process.env.REACT_APP_BACKEND_URL || '';
...
function buildKalshiUrl(eventTicker) {
  ...
  return `${KALSHI_TRADE_API_BASE}/api/kalshi/event/${eventTicker}`;
}
...
export default function TradesCenter() {
  ...
  const refresh = useCallback(async (silent = false) => {
    ...
    try {
      ...
      // ✅ FIX: Preserve live current_price/_hasLivePrice from previous state so
      // periodic DB reloads never flash +$0.00. If a trade already has a live
      // price (from the Kalshi poller), keep it instead of overwriting with the
      // stale DB value.
      setTrades(prev => {
        const prevMap = {};
        ...
        return data.map(t => {
          ...
          if (existing && isOpen(t) && existing._hasLivePrice) {
            const mergedPrice = existing.current_price;
            const mergedPnl = computePnl(t.side || 'yes', t.entry_price || 0, mergedPrice, t.quantity || 1);
            return { ...t, current_price: mergedPrice, pnl: mergedPnl, _hasLivePrice: true };
          }
          return t;
        });
      });
  ...
  const checkAutoExits = useCallback(async (tradesToCheck) => {
    ...
    for (const trade of openTrades) {
      // FIX: Only skip the grace period for stop-loss if current_price === entry_price
      // (i.e. no live price yet). If we already have a live price (_hasLivePrice), the
      // trade MUST be checked immediately — the grace period was protecting stale-/bin/sh.00
      // trades but was also letting already-breached trades escape for one full cycle.
      const tradeHasLivePrice = trade._hasLivePrice === true;
      if (newlyLoadedTradesRef.current.has(trade.id)) {
        if (!tradeHasLivePrice) {
          // No live price yet — skip this cycle to avoid false stop-loss on /bin/sh.00 data
          newlyLoadedTradesRef.current.delete(trade.id);
          continue;
        }
        // Live price is known — remove from grace set and evaluate immediately
        newlyLoadedTradesRef.current.delete(trade.id);
      }
      ...
      // FIX: Skip exit evaluation if no live price yet (current_price = entry_price means
      // data is stale from DB). This prevents false stop-loss triggers on /bin/sh.00 PnL AND
      // prevents missed triggers due to stale price hiding a real breach.
      if (!trade._hasLivePrice && trade.current_price === trade.entry_price) continue;
      ...
      // ✅ PRIORITY 2: STOP LOSS — FIRES IMMEDIATELY, NO GRACE, NO EXCEPTIONS.
      // Capital protection is absolute: the moment returnPct hits -stopLoss%, exit.
      else if (returnPct <= -exitRules.stopLoss) {
        shouldExit = true;
        // Cap exit price at exactly entry × (1 − stopLoss%) to prevent slippage recording.
        // Binary markets can collapse instantly (96¢ → 4¢); we record the guaranteed
        // threshold price, not the potentially much-worse market price.
        ...
        const slippageNote = returnPct < -exitRules.stopLoss
          ? ` (market at ${returnPct.toFixed(1)}%, capped at -${exitRules.stopLoss}%)`
          : '';
        exitReason = `Stop Loss: -${exitRules.stopLoss}% (exit at ${(exitPrice * 100).toFixed(1)}¢${slippageNote})`;
        console.log(`[STOP-LOSS] 🛑 ${trade.id} | ${returnPct.toFixed(2)}% ≤ -${exitRules.stopLoss}% | ${exitReason}`);
      }
        ...
        (async () => {
          ...
          const results = await Promise.allSettled(
            chunk.map(async (gameId) => {
              try {
                ...
                // ✅ FIX: Handle all response shapes (same as GameDetail.jsx)
                const event = json.event || json.series || json;
                const markets = event.markets || json.markets || [];
                
                if (markets.length >= 1) {
                  // ✅ FIX: markets[0] = home team (consistent with GameDetail + transformKalshiEvent)
                  // Previously used markets[1] which is the AWAY team — wrong market for home-side trades
                  const homeMarket = markets[0];
                  const bid = homeMarket.yes_bid ?? 0;
                  const ask = homeMarket.yes_ask ?? 100;
                  const livePrice = (bid + ask) / 2 / 100;
                  if (livePrice > 0) {
          ...
          if (Object.keys(updateMap).length > 0) {
            ...
            setTrades(prev => {
              ...
              return withLivePnl; // FIX: return array with recomputed PnL, not stale array
            });
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      const data = await fetchTrades(1000);
      // Preserve live current_price / pnl from previous state and build merged list.
      // CRITICAL: mergedData (not raw data) is passed to checkAutoExits so stop-loss
      // fires against LIVE prices, not stale DB prices where current_price = entry_price.
      let mergedData = data;
      setTrades(prev => {
        const prevMap = {};
        prev.forEach(t => { prevMap[t.id] = t; });
      ...
      setStats(calcPortfolioStats(mergedData));
      setLastUpdate(new Date());
      // FIX: Pass merged data (live prices) not raw DB data to checkAutoExits.
      setTimeout(() => checkAutoExits(mergedData), 50);
    }, 15_000);
  ...
  useEffect(() => {
    ...
    const fetchAllOnMount = async () => {
      try {
        ...
        await Promise.allSettled(
          gameIds.map(async (gameId) => {
            try {
              ...
              // FIX: markets[0] = home team; only need 1 market; handle all response shapes
              const event2 = json.event || json.series || json;
              const markets2 = event2.markets || json.markets || markets;
              if (markets2.length >= 1) {
                const homeMarket = markets2[0];
                const bid = homeMarket.yes_bid ?? 0;
                const ask = homeMarket.yes_ask ?? 100;
                const livePrice = (bid + ask) / 2 / 100;
        ...
        if (Object.keys(updateMap).length > 0) {
          // FIX: Hoist withLivePnl so it is accessible both inside setTrades AND in
          // the setTimeout below. Previously withLivePnl was scoped inside the setTrades
          // callback — so checkAutoExits(withLivePnl) was called with undefined.
          let snapshotForExitCheck = [];
          setTrades(prev => {
            ...
            setStats(calcPortfolioStats(withLivePnl));
            snapshotForExitCheck = withLivePnl; // hoist to outer scope
            return withLivePnl;
          });
          // Run stop-loss check immediately with LIVE prices after mount fetch.
          setTimeout(() => checkAutoExits(snapshotForExitCheck), 200);
        }
      } catch { /* ignore */ }
    };
    fetchAllOnMount();
